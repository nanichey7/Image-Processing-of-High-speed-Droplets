...
# Output text file
output_file = 'withmodifications.txt'
...
initial_frame_ids = set()  # stores ids of droplets that are present in the initial frame
droplet_positions = {}  # Track positions: {obj_id: [(x, y, frame_num), ...]}
impurity_ids = set()  # Track detected impurities
position_history_length = 10  # Number of frames to track position
movement_ratio_threshold = 0.02  # If movement is below 30% of average, mark as impurity
frame_volume_data = []
droplet_count_data = []
...
with open(output_file, 'w') as file:
    ...
    for result in results:
        ...
        # if it is the first frame don't consider the droplets as they are giving wrong results

        # note: added the second frame also to be just in case. can be deleted if not required

        if is_first_frame:
            for obj in result.boxes:
                initial_frame_ids.add(int(obj.id))
            is_first_frame = False
            continue

        # second frame is also excluded for safety
        if is_second_frame:
            for obj in result.boxes:
                initial_frame_ids.add(int(obj.id))
        ...
        for obj in result.boxes:
            ...
            if obj_id in impurity_ids:
                continue

            bbox = obj.xyxy.tolist()[0]  # Bounding box coordinates [x1, y1, x2, y2]
            # calculate the center position of each droplet
            center_x = (bbox[0] + bbox[2]) / 2
            center_y = (bbox[1] + bbox[3]) / 2

            # initialize or update position history
            if obj_id not in droplet_positions:
                droplet_positions[obj_id] = []
            droplet_positions[obj_id].append((center_x, center_y, total_frames))

            # maintain history length
            if len(droplet_positions[obj_id]) > position_history_length:
                droplet_positions[obj_id].pop(0)

            # check for impurity if enough position history is reached
            if len(droplet_positions[obj_id]) == position_history_length:
                # calculate average movement of all droplets

                all_movements = []
                for drop_id, positions in droplet_positions.items():
                    if len(positions) == position_history_length and drop_id not in impurity_ids:
                        all_movements.append(calculate_movement(positions))

                if all_movements:
                    avg_movement = np.mean(all_movements)
                    current_movement = calculate_movement(droplet_positions[obj_id])

                    if current_movement < (avg_movement * movement_ratio_threshold):
                        impurity_ids.add(obj_id)

                        # remove from tracking
                        if obj_id in object_histories:
                            del object_histories[obj_id]
                        if obj_id in droplet_positions:
                            del droplet_positions[obj_id]
                        if obj_id in recorded_ids:
                            recorded_ids.remove(obj_id)
                        continue
            ...
            # checking if the droplet is an impurity
            ...
            if not object_histories[obj_id]['stabilized']:
                ...
                if len(width_history) == history_length:
                    ...
                    if max_width_change <= size_change_threshold and max_height_change <= size_change_threshold:
                        ...
                        # calculate the volume only if they are not present in the first frame
                        if obj_id not in initial_frame_ids and obj_id not in impurity_ids:
                            ...
                            if mean_width <= chamber_height:
                                ...
                                print(
                                    f"id={obj_id}, sphere,diameter = {mean_width}, height = {h}, volume = {volume} pL\n")

                            elif mean_width > chamber_height and abs(mean_height - mean_width) < 0.175 * (
                                    mean_width + mean_height) / 2:
                                # Disk droplet
                                radius = (mean_width + mean_height) / 4
                                h = chamber_height
                                volume = ((2 * np.pi * (h / 2) * (radius - (h / 2)) ** 2) +
                                          (np.pi ** 2 * (h / 2) ** 2 * (radius - (h / 2)) * (
                                                      1 - 4 / (3 * np.pi)))) / 1000
                                print(
                                    f"id={obj_id}, disc,diameter = {2 * radius}, height = {h}, volume = {volume} pL\n")
        ...
        if stabilised_volumes: 
            ...
            mean_volumes = np.mean(stabilised_volumes)
            frame_volume_data.append((real_time,mean_volumes))
